    ...
    return(
        <div className="w-full flex items-center relative gap-1">
            {steps.map((step, index) => (
                <>
                    <div key={uuid()} className={`flex justify-center items-center rounded-full w-44 p-2 box-border aspect-square ${index+1 <= currentStep ? "bg-mainPurple" : "bg-gray-400"}`}>
                    ...
                    </div>
                    {
                        ...
                        <div key={uuid()} className={`h-1 w-full rounded-lg ${index+1 < currentStep ? "bg-mainPurple" : "bg-gray-400"}`}></div>
                    }
                </>
            ))}
        </div>
